...
actual object DeviceIntegrity {
    ...
    private fun hasSuspiciousPath(): Boolean =
        SUSPICIOUS_PATHS.any { runCatching { File(it).exists() }.getOrDefault(false) }

    private fun hasTestKeys(): Boolean =
        android.os.Build.TAGS?.contains("test-keys") == true

    private fun canWriteToSystem(): Boolean = runCatching {
        val f = File("/system/rancak_probe")
        f.createNewFile().also { if (it) f.delete() }
    }.getOrDefault(false)

    private fun isSuExecutable(): Boolean = runCatching {
