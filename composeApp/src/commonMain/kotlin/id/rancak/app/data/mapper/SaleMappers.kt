...
fun DeliveryResponseDto.toDomain() =
    Delivery(
        uuid = uuid,
...
fun QrPaymentDto.toDomain() =
    QrPayment(
        uuid = uuid,
        saleUuid = saleUuid,
        qrString = qrString,
        amount = amount.toDoubleOrNull()?.toLong() ?: 0L,
        status = QrPaymentStatus.from(status),
        expiresAt = expiresAt,
        usingWebhook = usingWebhook,
    )

fun RefundResponseDto.toDomain() =
    Refund(
        uuid = uuid,
        saleUuid = saleUuid,
...
fun RefundItemResponseDto.toDomain() =
    RefundItem(
...
fun CartItem.toSaleItemRequest() =
    SaleItemRequest(
        productUuid = productUuid,
        qty = qty,
