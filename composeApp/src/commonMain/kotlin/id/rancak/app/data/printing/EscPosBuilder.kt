...
object EscPosBuilder {
    ...
    private fun MutableList<Byte>.raw(vararg bytes: Int) = bytes.forEach { add(it.toByte()) }

    private fun MutableList<Byte>.text(s: String) = s.toByteArray(Charsets.UTF_8).forEach { add(it) }
    ...
    private fun paddedLine(
        ...
        return if (spaces <= 0) {
            ...
            leftText + " ".repeat(spaces) + rightText
        }
    }
    ...
    fun buildReceipt(data: ReceiptData): ByteArray {
        ...
        buf.text(data.storeName.uppercase())
        buf.lf()
        buf.doubleHeight(false)
        buf.bold(false)

        if (!data.storeAddress.isNullOrBlank()) {
            buf.text(data.storeAddress)
            buf.lf()
        }
        if (!data.storePhone.isNullOrBlank()) {
            buf.text(data.storePhone)
            buf.lf()
        }
        buf.lf()
        ...
        buf.divider('=')
        buf.lf()
        buf.text(paddedLine("#${data.invoiceNo}", data.orderType.uppercase()))
        buf.lf()
        if (!data.tableName.isNullOrBlank()) {
            buf.text("Meja  : ${data.tableName}")
            buf.lf()
        }
        if (!data.cashierName.isNullOrBlank()) {
            buf.text("Kasir : ${data.cashierName}")
            buf.lf()
        }
        buf.text(data.createdAt)
        buf.lf()
        buf.divider()
        buf.lf()
        ...
        for (item in data.items) {
            ...
            buf.text(paddedLine(qtyPrice, rupiah(item.subtotal)))
            buf.lf()
        ...
        // ── Totals ───────────────────────────────────────────────────────────
        buf.text(paddedLine("Subtotal", rupiah(data.subtotal)))
        buf.lf()
        if (data.discount > 0) {
            buf.text(paddedLine("Diskon", "-${rupiah(data.discount)}"))
            buf.lf()
        }
        if (data.surcharge > 0) {
            buf.text(paddedLine("Surcharge", rupiah(data.surcharge)))
            buf.lf()
        }
        if (data.tax > 0) {
            buf.text(paddedLine("Pajak", rupiah(data.tax)))
            buf.lf()
        }
        if (data.deliveryFee > 0) {
            buf.text(paddedLine("Ongkir", rupiah(data.deliveryFee)))
            buf.lf()
        }
        if (data.tip > 0) {
            buf.text(paddedLine("Tip", rupiah(data.tip)))
            buf.lf()
        }
        ...
        buf.bold(true)
        buf.doubleHeight(true)
        buf.text(paddedLine("TOTAL", rupiah(data.total)))
        buf.lf()
        buf.doubleHeight(false)
        buf.bold(false)
        ...
        if (!data.paymentMethod.isNullOrBlank()) {
            ...
            buf.text(paddedLine("Bayar ($method)", rupiah(data.paidAmount)))
            buf.lf()
            if (data.changeAmount > 0) {
                buf.text(paddedLine("Kembalian", rupiah(data.changeAmount)))
                buf.lf()
            }
        }
    ...
    fun buildKitchenTicket(data: KitchenTicketData): ByteArray {
        ...
        // ── Order info ───────────────────────────────────────────────────────
        buf.left()
        buf.text("No : ${data.invoiceNo}")
        buf.lf()
        buf.text("Tgl: ${data.createdAt}")
        buf.lf()
        buf.bold(true)
        buf.text("Tipe: ${data.orderType.uppercase()}")
        buf.lf()
        buf.bold(false)
        ...
        if (!data.customerName.isNullOrBlank()) {
            buf.text("Pelanggan: ${data.customerName}")
            buf.lf()
        }
        if (!data.cashierName.isNullOrBlank()) {
            buf.text("Kasir: ${data.cashierName}")
