...
class AdminRepositoryImpl(
    ...
    override suspend fun getSurcharge(surchargeId: String): Resource<Surcharge> =
        safe(
    ...
    override suspend fun createSurcharge(
        ...
        safe(
            block = {
                api.createSurcharge(
                    tenantUuid,
                    CreateSurchargeRequest(orderType, name, amount, isPercentage, maxAmount, sortOrder),
                )
            },
            map = { it.toDomain() },
        ...
        safe(
            block = {
                api.updateSurcharge(
                    tenantUuid,
                    surchargeId,
                    UpdateSurchargeRequest(name, amount, isPercentage, maxAmount, isActive, sortOrder),
                )
            },
            map = { it.toDomain() },
    ...
    override suspend fun deleteSurcharge(surchargeId: String): Resource<Unit> =
        safeUnit(
    ...
    override suspend fun getVoucher(voucherId: String): Resource<Voucher> =
        safe(
    ...
    override suspend fun deleteVoucher(voucherId: String): Resource<Unit> =
        safeUnit(
    ...
    override suspend fun getTaxConfig(configId: String): Resource<TaxConfig> =
        safe(
    ...
    override suspend fun deleteTaxConfig(configId: String): Resource<Unit> =
        safeUnit(
    ...
    override suspend fun getTable(tableId: String): Resource<Table> =
        safe(
    ...
    override suspend fun deleteTable(tableId: String): Resource<Unit> =
        safeUnit(
    ...
    override suspend fun getBundle(bundleId: String): Resource<Bundle> =
        safe(
            ...
            block = {
                api.createBundle(
                    ...
                    CreateBundleRequest(
                        ...
                        price,
                        items.map {
                            BundleItemRequest(it.productUuid, it.qty)
                        },
                        description,
                        sku,
                        isActive,
    ...
    override suspend fun deleteBundle(bundleId: String): Resource<Unit> =
        safeUnit(
    ...
    override suspend fun getModifiers(): Resource<List<Modifier>> =
        safeList(
            block = { api.getModifiers(tenantUuid) },
            errorMsg = "Gagal memuat daftar modifier",
        ) { it.toDomain() }

    override suspend fun getModifier(modifierId: String): Resource<Modifier> =
        safe(
    ...
    override suspend fun createModifier(
        name: String,
        sortOrder: Int,
        isActive: Boolean,
    ): Resource<Modifier> =
    ...
    override suspend fun deleteModifier(modifierId: String): Resource<Unit> =
        safeUnit(
    ...
    override suspend fun getDiscountRule(ruleId: String): Resource<DiscountRule> =
        safe(
    ...
    override suspend fun createDiscountRule(
        ...
        safe(
            block = {
                api.createDiscountRule(
                    tenantUuid,
                    CreateDiscountRuleRequest(name, description, ruleType, discountType, discountValue, startTime, endTime, applicableDays, minPurchaseAmount, priority, stackable, maxDiscount, isActive),
                )
            },
            map = { it.toDomain() },
    ...
    override suspend fun updateDiscountRule(
        ...
        safe(
            block = {
                api.updateDiscountRule(
                    tenantUuid,
                    ruleId,
                    UpdateDiscountRuleRequest(update.name, update.description, update.ruleType, update.discountType, update.discountValue, update.startTime, update.endTime, update.applicableDays, update.minPurchaseAmount, update.priority, update.stackable, update.maxDiscount, update.isActive),
                )
            },
            map = { it.toDomain() },
    ...
    override suspend fun deleteDiscountRule(ruleId: String): Resource<Unit> =
        safeUnit(
    ...
    override suspend fun getVariantGroups(productId: String): Resource<List<VariantGroup>> =
        safe(
    ...
    override suspend fun deleteVariantGroup(
        productId: String,
        groupId: String,
    ): Resource<Unit> =
        safeUnit(
        ...
        safe(
            block = {
                api.createVariant(
                    ...
                    productId,
                    groupId,
                    CreateVariantRequest(name, priceAdjustment, isDefault, isActive, sortOrder),
                )
            },
            map = { it.toDomain() },
        ...
        safe(
            block = {
                api.updateVariant(
                    ...
                    groupId,
                    variantId,
                    UpdateVariantRequest(name, priceAdjustment, isDefault, isActive, sortOrder),
                )
            },
            map = { it.toDomain() },
    ...
    override suspend fun adjustStock(
        ...
        safe(
            ...
            map = {
                    dto ->
                StockAdjustmentResult(dto.productUuid, dto.productName, dto.stockBefore, dto.stockAfter, dto.adjustmentType, dto.quantity)
            },
            errorMsg = "Gagal menyesuaikan stok",
        )

    override suspend fun getProductBatches(productId: String): Resource<List<ProductBatch>> =
        safe(
    ...
    override suspend fun createProductBatch(
        ...
        safe(
            block = {
                api.createProductBatch(
                    tenantUuid,
                    productId,
                    CreateProductBatchRequest(quantity, expiryDate, costPrice, batchNumber, note, receivedAt),
                )
            },
            map = { it.toDomain() },
    ...
    override suspend fun createProduct(
        ...
        safe(
            block = {
                api.createProduct(
                    tenantUuid,
                    CreateProductRequest(name, price, description, sku, barcode, categoryUuid, unit, stock, hasExpiry, isActive),
                )
            },
            map = { it.toDomain() },
        ...
        safe(
            block = {
                api.updateProduct(
                    tenantUuid,
                    productId,
                    UpdateProductRequest(name, price, description, sku, barcode, categoryUuid, unit, isActive),
                )
            },
            map = { it.toDomain() },
    ...
    override suspend fun updateReceiptSettings(update: ReceiptSettingsUpdate): Resource<ReceiptSettingsConfig> =
        safe(
            block = {
                api.updateReceiptSettings(
                    tenantUuid,
                    UpdateReceiptSettingsRequest(update.logoUrl, update.email, update.website, update.npwp, update.receiptHeader, update.receiptFooter, update.receiptFooter2, update.logoPosition, update.logoSizePct, update.receiptNameSize, update.separatorStyle, update.separatorCount, update.footerPosition, update.receiptInstagram, update.receiptFacebook, update.receiptWifiSsid, update.receiptWifiPassword),
                )
            },
            map = { it.toDomain() },
