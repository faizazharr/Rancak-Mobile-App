...
class GroupsRepositoryImpl(
    ...
    override suspend fun getGroup(groupUuid: String): Resource<Group> =
        safe(
    ...
    override suspend fun deleteGroup(groupUuid: String): Resource<Unit> =
        safeUnit(
            block = { api.deleteGroup(groupUuid) },
            errorMsg = "Gagal menghapus grup",
        )

    override suspend fun getGroupTenants(groupUuid: String): Resource<List<Tenant>> =
        safe(
    ...
    override suspend fun assignTenantToGroup(
        groupUuid: String,
        tenantUuid: String,
    ): Resource<Unit> =
        safeUnit(
    ...
    override suspend fun removeTenantFromGroup(
        groupUuid: String,
        tenantUuid: String,
    ): Resource<Unit> =
        safeUnit(
    ...
    override suspend fun getGroupBranches(
        groupUuid: String,
        start: String?,
        end: String?,
    ): Resource<List<BranchReport>> =
        safe(
    ...
    override suspend fun getGroupRevenueSeries(
        groupUuid: String,
        start: String?,
        end: String?,
    ): Resource<String> = safeJson { api.getGroupRevenueSeries(groupUuid, start, end) }

    override suspend fun getGroupTopProducts(groupUuid: String): Resource<String> = safeJson { api.getGroupTopProducts(groupUuid) }

    override suspend fun getGroupPaymentMethods(groupUuid: String): Resource<String> = safeJson { api.getGroupPaymentMethods(groupUuid) }

    override suspend fun getGroupPeakHours(groupUuid: String): Resource<String> = safeJson { api.getGroupPeakHours(groupUuid) }

    override suspend fun getGroupPnl(groupUuid: String): Resource<String> = safeJson { api.getGroupPnl(groupUuid) }

    override suspend fun getGroupCategories(groupUuid: String): Resource<String> = safeJson { api.getGroupCategories(groupUuid) }

    override suspend fun getGroupCashiers(groupUuid: String): Resource<String> = safeJson { api.getGroupCashiers(groupUuid) }

    override suspend fun getGroupDiscounts(groupUuid: String): Resource<String> = safeJson { api.getGroupDiscounts(groupUuid) }

    override suspend fun getGroupVoidRefund(groupUuid: String): Resource<String> = safeJson { api.getGroupVoidRefund(groupUuid) }

    private suspend fun safeJson(block: suspend () -> ApiResponse<JsonElement>): Resource<String> =
        try {
            val response = block()
            if (response.isSuccess && response.data != null) {
                Resource.Success(response.data.toString())
            } else {
                Resource.Error(response.message ?: "Gagal memuat data")
            }
        } catch (e: Exception) {
