...
class ProductRepositoryImpl(
    ...
    override suspend fun getProductsFromCache(
        query: String?,
        categoryId: String?,
    ): Resource<List<Product>> {
    ...
    override suspend fun getProductByBarcode(barcode: String): Resource<Product> {
        return try {
            ...
            if (response.isSuccess && response.data != null) {
                ...
                if (local != null) {
                    Resource.Success(local.toDomain())
                } else {
            ...
            if (local != null) {
                ...
                Resource.Error(e.toNetworkMessage("Tidak ada koneksi internet"))
            }
        }
    }
    ...
    override suspend fun unmark86(productUuid: String): Resource<Unit> {
        ...
        return try {
            ...
            if (response.isSuccess || response.statusCode == 404) {
                ...
                Resource.Error(response.message ?: "Gagal membatalkan produk 86")
            }
        } catch (e: Exception) {
