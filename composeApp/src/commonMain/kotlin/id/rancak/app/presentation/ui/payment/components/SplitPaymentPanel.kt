...
internal fun SplitPaymentPanel(
    ...
    val confirmedQtyMap: ImmutableMap<Int, Int> =
        remember(splitGroups) {
            buildMap<Int, Int> {
    ...
    val currentItemSubtotal: Long =
        remember(currentItemQtys, items) {
    ...
    val groupActualTotal: Long =
        remember(currentItemSubtotal, orderItemSubtotal, orderTotal) {
    ...
    val change =
        if (currentMethod == PaymentMethod.CASH && cashPaid >= groupActualTotal) {
            cashPaid - groupActualTotal
        } else {
            0L
        }
    val canConfirm =
