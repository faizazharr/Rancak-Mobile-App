...
class ProductRepositoryImplTest {
    ...
    @Test
    fun `getProducts - API success returns products`() =
    ...
    @Test
    fun `getProducts - API success caches full list to local DAO`() =
        kotlinx.coroutines.test.runTest {
    ...
    @Test
    fun `getProducts - API failure with populated cache serves cache as Success`() =
        kotlinx.coroutines.test.runTest {
            val dao = FakeProductDao()
            // Pre-seed cache
            makeRepo(productListJson, productDao = dao).getProducts()
    ...
    @Test
    fun `getProducts - API failure with empty cache returns Resource_Error`() =
        kotlinx.coroutines.test.runTest {
    ...
    @Test
    fun `getProducts - API error response non-2xx with empty cache returns Resource_Error`() =
        kotlinx.coroutines.test.runTest {
    ...
    @Test
    fun `getProducts - filtered query result is NOT cached to DAO`() =
        kotlinx.coroutines.test.runTest {
            val dao = FakeProductDao()
    ...
    @Test
    fun `getProducts - filtered query falls back to local search on API failure`() =
        kotlinx.coroutines.test.runTest {
            val dao = FakeProductDao()
    ...
    @Test
    fun `getProductByBarcode - API success returns product`() =
        kotlinx.coroutines.test.runTest {
    ...
    @Test
    fun `getProductByBarcode - API failure falls back to cached barcode match`() =
        kotlinx.coroutines.test.runTest {
            val dao = FakeProductDao()
    ...
    @Test
    fun `getCategories - API success returns and caches categories`() =
        kotlinx.coroutines.test.runTest {
    ...
    @Test
    fun `getCategories - API failure with populated cache returns cached categories`() =
        kotlinx.coroutines.test.runTest {
            val catDao = FakeCategoryDao()
            // Prime cache
    ...
    @Test
    fun `getCategories - API error with empty cache returns Resource_Error`() =
        kotlinx.coroutines.test.runTest {
            val repo =
                ProductRepositoryImpl(
                    mockApiService { throw Exception("No network") },
    ...
    @Test
    fun `getProducts - throws when no tenant selected`() =
        kotlinx.coroutines.test.runTest {
            val repo =
                ProductRepositoryImpl(
